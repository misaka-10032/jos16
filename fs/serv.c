...
serve_read(envid_t envid, union Fsipc *ipc)
{
  ...
  struct OpenFile *of;
  int rc = openfile_lookup(envid, req->req_fileid, &of);
  if (rc < 0)
    return rc;

  size_t cnt = MIN(PGSIZE, req->req_n);
  rc = file_read(of->o_file, (void *) ret->ret_buf,
                 cnt, of->o_fd->fd_offset);
  if (rc < 0)
    return rc;

  of->o_fd->fd_offset += rc;
	return rc;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
  ...
  size_t cnt = MIN(sizeof(req->req_buf), req->req_n);
  rc = file_write(of->o_file, req->req_buf, cnt, of->o_fd->fd_offset);
  if (rc < 0)
    return rc;
